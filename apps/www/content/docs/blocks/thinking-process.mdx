...
- **组件定位**：展示一个“思考步骤块”（状态 + 标题 + 可折叠内容）。
- **组合方式**：如需展示子步骤列表，推荐在 `content` 中渲染（例如渲染 `ThinkingStepItem` 列表）。
...
### 默认开合策略（不传 defaultOpen/open 时）

- `thinking`：默认展开（longRunning 场景除外）
- `completed`：默认收起
- `cancelled`：默认展开（内容不会自动收起）

### 时间显示规则

- 仅当 `status === "completed"` 且 `duration` 存在时显示耗时
- 当内容中存在工具调用统计文案（例如 `contentBlocks` 的 `subSteps` 中包含工具调用并显示“已使用 n 个工具”）时，不显示耗时
...
### 使用 contentBlocks（推荐：文字 + 子步骤穿插）

当你需要在内容区域实现「文字 → 子步骤列表 → 文字」这种穿插渲染时，推荐使用 `contentBlocks`：
...
<ThinkingStep
  ...
  contentBlocks={[
    { type: "text", key: "intro", content: "下面是关键子步骤：" },
    {
      ...
      key: "steps",
      steps: [
        { key: "s1", status: "success", title: "步骤 1", items: [{ key: "i1", content: "已完成" }] },
        { key: "s2", status: "loading", title: "步骤 2", items: [{}] },
      ],
    },
    { type: "text", key: "outro", content: "步骤完成后，将生成最终结论。" },
  ]}
  defaultOpen
/>
```
...
- `contentBlocks` 存在时，将忽略 `content / subSteps / renderSubSteps`，按 blocks 顺序渲染。
- `subSteps` block 默认会渲染为 `ThinkingStepItemContainerPrimitive + ThinkingStepItem`（无需手写 JSX）。
...
| `contentBlocks` | `Array<{ type: 'text' \| 'subSteps' \| 'node'; ... }>` | - | 内容块（推荐）：支持文字与子步骤穿插渲染 |
| `duration` | `number` | - | 耗时（秒） |
| `icon` | `React.ReactNode` | - | 自定义图标 |
| `arrowIcon` | `React.ReactNode` | - | 自定义箭头图标 |
| `hint` | `React.ReactNode` | - | 长耗时提示文案（无内容时显示在 header 下方，可折叠） |
| `longRunning` | `boolean` | `false` | 长耗时场景：默认收起；未提供 `hint` 时使用默认提示文案 |
...
可选容器组件，用于纵向排列多个 `ThinkingStep` 并提供统一间距。
