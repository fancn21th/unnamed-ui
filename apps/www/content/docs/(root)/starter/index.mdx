...
1. [环境准备与安装](#1-环境准备与安装)
   ...
   - [1.5 Token 定制说明](#15-token-定制说明)
2. [实战演练：搭建 Proposal 页面](#2-实战演练搭建-proposal-页面)
3. [核心功能：对话与流式输出](#3-核心功能对话与流式输出)

**延伸阅读**：若您从 Figma 复制样式时看到 `var(--Container-bg-brand-light-hover)` 等形式，可参阅 [设计变量使用指南](/docs/design-tokens) 了解如何在 Tailwind 中正确使用。
...
样式安装后，在 `src/index.css`（或 `app/globals.css`）**顶部追加**引入：
...
如需自定义主题色，请新建 `theme.css`，在入口 CSS 中 globals 引入**之后**追加引入：
...
**不要直接修改 `globals.css`**，以便后续升级 Unnamed UI 样式时不受影响。变量名可参考 [设计变量使用指南](/docs/design-tokens)。也可使用主题工作台（Theme Studio）调整后复制生成的 CSS 到 `theme.css`。
