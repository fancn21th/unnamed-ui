...
通过 `action` prop 完全自定义右侧操作区域，替换默认的 ellipsis 菜单。

```tsx
<ReportCard
  title="自定义操作"
  description="完全自定义右侧区域"
  action={
    <button className="px-3 py-1 rounded-md bg-[var(--text-brand)] text-white text-sm cursor-pointer">
      操作
    </button>
  }
/>
...
可以在自定义操作区域的同时保留默认操作行为。

```tsx
<ReportCard
  title="混合模式"
  description="自定义图标 + 默认菜单"
  action={
    <div className="flex items-center gap-2">
