...
const parseDynamicFormSchema = (raw: string): FormSchema | null => {
  const trimmed = raw.trim();
  const normalized = trimmed.startsWith("schema:")
    ? trimmed.replace(/^schema:\s*/i, "")
    : trimmed;
  try {
    return JSON.parse(normalized) as FormSchema;
  } catch {
    return null;
  }
};
...
export const Code: React.FC<ComponentProps<"code">> = (props) => {
  ...
  if (lang === "dynamic-form") {
    const schema = parseDynamicFormSchema(children) ?? { fields: [] };
