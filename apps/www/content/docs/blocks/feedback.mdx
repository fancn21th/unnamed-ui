...
| `selectedId` | `string` | - | 受控模式：当前选中的选项 ID |
| `defaultSelectedId` | `string` | `""` | 非受控模式：默认选中的选项 ID |
| `onSelect` | `(id: string) => void` | - | 选项选择变化时的回调 |
...
| `onSubmit` | `(payload: { selectedId: string; inputValue: string }) => void` | - | 表单提交时的回调 |
...
- 受控模式下需要同时提供 `selectedId` 和 `onSelect`、`inputValue` 和 `onInputChange`
- 非受控模式下可以使用 `defaultSelectedId` 和 `defaultInputValue` 设置初始值
- `onSubmit` 回调会接收包含 `selectedId` 和 `inputValue` 的对象
